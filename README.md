...
- 内部ではstd::arrayで固定長バッファを持つ `FrozenString` クラスを定義し、文字列リテラルや数値などからコンパイル時に文字列を生成できます（C++20 コンパイル時関数 `consteval`）
...
- `minify_cypher(input, output, output_size)` : Cypher クエリを実行時に固定バッファへ縮小します。書き込んだ長さを返します
  - `minify_cypher(input)` : Cypher クエリ文字列リテラルをコンパイル時に縮小し、`FrozenString<N>` 型を返します
...
なお、`minify_cypher` は `include/frozenchars/minify.hpp` で提供され、アンブレラ `frozenchars.hpp` から利用できます。

```cpp
#include "frozenchars/minify.hpp"
using namespace frozenchars;

auto constexpr q = minify_cypher(
  "MATCH (n) // comment\n"
  "RETURN n;");
static_assert(q.sv() == "MATCH(n)RETURN n");

// 文字列リテラル内の // や /* */ は削除しない
auto constexpr s = minify_cypher("RETURN '//not a comment'");
static_assert(s.sv() == "RETURN'//not a comment'");

// バッククォート識別子内の // は削除しない
auto constexpr b = minify_cypher("MATCH (`my node`) RETURN `my node`");
...
- **CHD 2段ハッシュ（65キー以上）**:
  キー数が64を超えると単一シードの完全ハッシュ探索が困難になるため、CHD（bucket displacement）方式の2段ハッシュに自動的に切り替わります。キー数によらず O(1)（ハッシュ2回 + 比較1回）で検索できます。
- **データローカリティと宣言順反復**:
...
| キー数上限 | なし（64超は CHD 2段ハッシュ） | 127 | なし | なし |
...
| キー数 > 64 | `frozen_map` | CHD 2段ハッシュで O(1) を維持 |
...
> 長さフィルタ + CHD 2段ハッシュ（O(1)）で実行されます。委譲条件は以下のすべてを満たす場合です：
