...
template <size_t N>
[[nodiscard]] constexpr auto parse_iso_date(FrozenString<N> const& s) noexcept -> std::chrono::year_month_day
{
  if (s.length != 10) return {};
  if (s.buffer[4] != '-') return {};
  if (s.buffer[7] != '-') return {};
  auto const m = detail::decode_two_digits(s.buffer[5], s.buffer[6]);
  if (m < 1 || m > 12) return {};
  auto const d = detail::decode_two_digits(s.buffer[8], s.buffer[9]);
  if (d < 1 || d > 31) return {};
...
template <size_t N>
[[nodiscard]] constexpr auto parse_iso_datetime(FrozenString<N> const& s) noexcept -> std::chrono::sys_seconds
{
  using namespace std::chrono;
  if (s.length < 19) return {};
  if (s.length != 19 && s.length != 20 && s.length != 25) return {};
  if (s.buffer[4] != '-') return {};
  if (s.buffer[7] != '-') return {};
  if (s.buffer[10] != 'T') return {};
  if (s.buffer[13] != ':') return {};
  if (s.buffer[16] != ':') return {};
  auto const h = detail::decode_two_digits(s.buffer[11], s.buffer[12]);
  auto const mi = detail::decode_two_digits(s.buffer[14], s.buffer[15]);
  auto const sec = detail::decode_two_digits(s.buffer[17], s.buffer[18]);
  if (h > 23) return {};
  ...
  if (sec > 59) return {};
  if (s.length >= 20) {
    auto const tz = s.buffer[19];
    if (tz != 'Z' && tz != 'z' && tz != '+' && tz != '-') return {};
  }
  if (s.length == 25) {
    if (s.buffer[22] != ':') return {};
  }
  auto const date = detail::parse_ymd_iso(s.data(), 10);
  auto const time = detail::parse_hms(s.data() + 11, 8);
  auto offset = minutes{0};
...
template <size_t N>
[[nodiscard]] constexpr auto parse_date_macro(FrozenString<N> const& s) noexcept -> std::chrono::year_month_day
{
  if (s.length != 11) return {};
  if (s.buffer[6] != ' ') return {};
  auto const mon = detail::month_name_to_number(s.data(), 3);
  if (mon < 1 || mon > 12) return {};
  auto const day_val = (s.buffer[4] == ' ')
    ? static_cast<unsigned>(s.buffer[5] - '0')
    : detail::decode_two_digits(s.buffer[4], s.buffer[5]);
  if (day_val < 1 || day_val > 31) return {};
  auto const year_val = detail::decode_four_digits(s.buffer[7], s.buffer[8], s.buffer[9], s.buffer[10]);
  if (year_val < 1970) return {};
  return detail::parse_ymd_macro(s.data(), 11);
}
