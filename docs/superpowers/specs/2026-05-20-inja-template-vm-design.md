...
- 実行時処理: `template_vm<Src>::render(template_value const&) -> std::string`
...
- 数値演算 `+ - * /`:
  - 両辺が数値なら数値演算
  - `int64` 同士かつ `/` 以外は `int64` を維持、`/` は `double`
  - `double` を含む場合は `double`
- `%`: 両辺 `int64` のみ許可（それ以外は実行時エラー）
- `+` の文字列連結:
  - 両辺が文字列なら連結
  - 片側だけ文字列は未サポート（実行時エラー）
- 比較演算:
  - 同種同士の比較のみ許可（数値は `int64`/`double` を数値比較として許可）
- `in`:
  - 左が文字列・右が文字列: 部分文字列判定
  - 右が配列: 等価要素の存在判定
  - 右がオブジェクト: 左が文字列のときキー存在判定
  - 上記以外は実行時エラー
...
- `bool` は `bool` とだけ等価
- 文字列は文字列同士のみ等価
- 配列/オブジェクトは深い等価比較は行わず、同型同士でも比較不許可（実行時エラー）
- 数値は `int64`/`double` 混在を許可し、`double` へ正規化して比較
- `in` の配列要素比較も上記規則をそのまま使う

## 7. `for` 反復仕様

- 配列: `{% for item in arr %}` をサポート（`item` に要素を束縛）
- オブジェクト:
  - `{% for value in obj %}`（値のみ）
  - `{% for key, value in obj %}`（キー値ペア）
- オブジェクトの反復順は `std::unordered_map` の実装順に従い未規定とする
- `loop` 変数は `index`(0-based), `index1`(1-based), `is_first`, `is_last` を提供する
- アクセス記法は `loop.index`, `loop.index1`, `loop.is_first`, `loop.is_last`
- `loop` の有効範囲はループ本文ブロック内のみ
- ネストループ時は内側 `loop` が外側をシャドーし、`loop.parent.*` は初期実装では未対応
- ループ変数名が既存変数と衝突した場合はループ本文内でループ変数を優先し、ブロック終了後に元の束縛へ復帰
